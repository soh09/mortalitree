...
CKPT_BASE = "/checkpoints"          # ckpt volume root (Stage C reads/writes here)
NEON_DIR  = "/neon"
CLAY_CKPT = "/checkpoints/clay-v1.5.ckpt"

# Stage-B checkpoints to finetune in Stage C (upload these to the clay-checkpoints
# volume root: `modal volume put clay-checkpoints clay/checkpoints/<f> /<f>`).
STAGE_B_CKPTS = {
    "q150":    f"{CKPT_BASE}/q150_final.pt",
    "q500":    f"{CKPT_BASE}/q500_final.pt",
    "dqdetr":  f"{CKPT_BASE}/dq-detr_final.pt",
    "quarter": f"{CKPT_BASE}/quarter_final.pt",
}
...
# Stage C mounts the checkpoint volume at its ROOT (/checkpoints) so the Clay
# encoder, the uploaded Stage-B checkpoints, and the Stage-C outputs all live on
# the persisted volume.
COMMON_C = dict(
    gpu="A10G",
    timeout=60 * 60 * 24,
    volumes={DATA_DIR: data_vol, CKPT_BASE: ckpt_vol},
    secrets=_wandb_secret,
)
...
def _build_for_stage_c(model_type: str, config_path: str):
    """Build the detector for Stage C by model_type.

    q150 / q500 -> FixedQueryTreeDetector (fixed object queries, plain detection
    loss). dqdetr -> the DQ-DETR TreeDetector (DQS + CCM + CGFE). All share the
    Clay encoder loaded from CLAY_CKPT; the model_type's Stage-B checkpoint is
    loaded later inside run_stage_c_multi. Returns (model, cfg, is_dqdetr)."""
    import sys
    import torch
    import yaml
    ...
    if model_type == "quarter":
        # The quartering model is the same fixed-query detector run on 128px
        # quarters; num_queries comes from its Stage-B checkpoint (config-free).
        ck = torch.load(STAGE_B_CKPTS["quarter"], map_location="cpu", weights_only=False)
        sd = ck.get("model", ck)
        num_queries = sd["head.queries.weight"].shape[0]
    else:
        num_queries = {"q150": 150, "q500": 500}.get(model_type)
    if num_queries is None:
        raise ValueError(f"Unknown model_type '{model_type}' (expected q150|q500|dqdetr|quarter)")
    from src.model.detector_fixed import FixedQueryTreeDetector
    model = FixedQueryTreeDetector(
        encoder,
        neck_in_channels=m["neck_in_channels"],
        neck_out_channels=m["neck_out_channels"],
...
# ---------------------------------------------------------------------------
# Stage C — unified finetuning for q150 / q500 / dqdetr
# ---------------------------------------------------------------------------
@app.function(**COMMON_C)
def stage_c(
    model: str = "dqdetr",
    ...
    """Finetune one of the detectors on the NAIP good-tile split.

    model: which detector to train — "q150" / "q500" (fixed-query
    FixedQueryTreeDetector, detection loss only), "dqdetr" (the DQ-DETR
    TreeDetector with DQS + CCM + CGFE), or "quarter" (fixed-query detector trained
    on 128px quarters with stitched parent-level eval). The matching Stage-B
    checkpoint is taken from STAGE_B_CKPTS unless `stage_b_checkpoint` is given.

    Logging: train AND val f1/precision/recall/mAP/count + losses each epoch,
    per-epoch prediction-vs-GT panels on fixed train + val tiles (plus CCM density
    overlays for dqdetr). Checkpoints land in /checkpoints/stage_c as
    {model}_stage_c_epoch{NNN}.pt every `ckpt_every` epochs, with rolling
    {model}_stage_c_best.pt / _final.pt.

    cgfe_start_epoch (dqdetr only): epoch to switch CGFE on; default 0 (on from
    the start, since Stage B ended with CGFE on). Pass -1 to use the config value.
    Ignored for the fixed-query models."""
    _ensure_clay_ckpt()
    sb_ckpt = stage_b_checkpoint or STAGE_B_CKPTS[model]
    mdl, cfg, is_dqdetr = _build_for_stage_c(model, config)
    ...
    cgfe = (t.get("cgfe_start_epoch") if cgfe_start_epoch < 0 else cgfe_start_epoch)

    run = _wandb_init(f"stage_c_{model}", cfg, extra={"stage": "C", "model": model})
    try:
        if model == "quarter":
            # Quartered training: 256 tiles -> 4x128 quarters, stitched eval.
            from src.train.stage_c_quarter import run_stage_c_quarter
            run_stage_c_quarter(
                model=mdl,
                ckpt_prefix=model,
                train_annotations_path=train_annotations,
                val_annotations_path=val_annotations,
                checkpoint_dir=f"{CKPT_BASE}/stage_c",
                ...
                device="cuda",
                num_workers=t["num_workers"],
                stage_b_checkpoint=sb_ckpt,
                viz_every=viz_every,
                n_viz_tiles=n_viz_tiles,
                viz_conf_thresh=viz_conf_thresh,
                eval_conf_thresh=ev.get("conf_thresh", 0.5),
                eval_iou_thresh=ev.get("iou_thresh", 0.5),
                ckpt_every=ckpt_every,
                on_checkpoint=ckpt_vol.commit,
            )
        else:
            from src.train.stage_c_multi import run_stage_c_multi
            run_stage_c_multi(
                model=mdl,
                is_dqdetr=is_dqdetr,
                ckpt_prefix=model,
                ...
                lam_ccm=l.get("lam_ccm", 1.0),
                lam_enc=l.get("lam_enc", 1.0),
                cgfe_start_epoch=(cgfe if is_dqdetr else None),
                device="cuda",
                num_workers=t["num_workers"],
                stage_b_checkpoint=sb_ckpt,
                viz_every=viz_every,
                ...
                eval_iou_thresh=ev.get("iou_thresh", 0.5),
                ckpt_every=ckpt_every,
                on_checkpoint=ckpt_vol.commit,   # persist best / rolling ckpts immediately
            )
    finally:
        if run is not None:
            run.finish()
    ckpt_vol.commit()
    print(f"Stage C ({model}) complete. Checkpoints -> {CKPT_BASE}/stage_c/{model}_stage_c_*.pt")
...
def main(
    stage: str = "c",
    models: str = "q150,q500,dqdetr,quarter",
    data_source: str = "neon",
    out_tag: str = "patches",
    stage_b_annotations: str = "/data/deepforest.json",
    train_annotations: str = "/data/stage_c/train.json",
    ...
    """stage: "b" (Stage B), "c" (Stage C), or "both".

    For Stage C, `models` is a comma-separated subset of q150,q500,dqdetr,quarter;
    each is launched as its own GPU job (in parallel via .spawn). Run a single model
    with `modal run modal_train.py::stage_c --model q150`. The `quarter` model trains
    on 128px quarters with stitched parent-level eval (needs quarter_final.pt)."""
    if stage in ("b", "both"):
    ...
    if stage in ("c", "both"):
        for mt in [s.strip() for s in models.split(",") if s.strip()]:
            print(f"Launching Stage C for model={mt}...")
            stage_c.spawn(
                model=mt,
                train_annotations=train_annotations,
                val_annotations=val_annotations,
            )
        print("Stage C jobs spawned. Track them in the Modal dashboard / W&B.")
